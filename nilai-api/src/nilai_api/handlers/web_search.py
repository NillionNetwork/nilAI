...
async def enhance_messages_with_web_search(messages: List[Message], query: str) -> Tuple[List[Message], List[Dict[str, str]]]:
    """
    ...
    search_results, sources = await loop.run_in_executor(None, lambda: perform_web_search_sync(query))
...
async def handle_web_search(req_messages: List[Message]) -> Tuple[List[Message], List[Dict[str, str]]]:
    """
    ...
    try:
        enhanced_messages, sources = await enhance_messages_with_web_search(req_messages, user_query)
        return enhanced_messages, sources
    except Exception as e:
