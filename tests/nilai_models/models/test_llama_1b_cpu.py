...
from tests import model_metadata, model_endpoint, response as RESPONSE
...
async def test_chat_completion_empty_messages(llama_model):
    ...
    llama_model.chat_completion.side_effect = HTTPException(status_code=400, detail="The 'messages' field is required.")
...
async def test_chat_completion_missing_model(llama_model):
    ...
    llama_model.chat_completion.side_effect = HTTPException(status_code=400, detail="The 'model' field is required.")
    with pytest.raises(HTTPException) as exc_info:
        await llama_model.chat_completion(req)
    assert exc_info.value.status_code == 400
