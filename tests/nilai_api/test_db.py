from tests.nilai_api import mock_db
...
def test_insert_user(mock_db, event_loop):
    ...
    user = event_loop.run_until_complete(mock_db.insert_user("Test User", "test@example.com"))
...
def test_query_logging(mock_db, event_loop):
    ...
    event_loop.run_until_complete(mock_db.log_query(user['userid'], "test-model", 10, 15))
