...
MODEL_HEALTH_STATUS=$(docker inspect --format='{{.State.Health.Status}}' nilai-gpt_20b_gpu 2>/dev/null)
...
if [ "$MODEL_HEALTH_STATUS" != "healthy" ]; then
    echo "Error: nilai-gpt_20b_gpu failed to become healthy after $MAX_ATTEMPTS attempts"
    exit 1
fi
